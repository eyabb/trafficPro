...
@Endpoint
public class WeatherForDrivingEndpoint {

  private static final String NAMESPACE_URI = "http://example.com/weather";
  private static final Logger logger = LoggerFactory.getLogger(WeatherForDrivingEndpoint.class);

  private final WeatherForDrivingService weatherSoapService;
  private final RestTemplate restTemplate = new RestTemplate();

  @Autowired
  public WeatherForDrivingEndpoint(WeatherForDrivingService weatherSoapService) {
    this.weatherSoapService = weatherSoapService;
  }

  @PayloadRoot(namespace = NAMESPACE_URI, localPart = "ComputeWeatherRequest")
  @ResponsePayload
  public ComputeWeatherResponse computeWeather(@RequestPayload ComputeWeatherRequest request) {
    ...
    try {
      ...
      logger.error("Exception occurred during computeWeather: {}", e.getMessage(), e);
    }
